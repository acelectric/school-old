...
<?php if(!empty($datas)):?>
      ...
      <div class="card w-75 mx-auto mt-5">
        ...
        <div class="card-body">
          ...
          <?php 
            $hashlist = array("c","c++","c#","java","html","css","js","python","php");
            $hashtag_sql = "SELECT * FROM `hashtag` WHERE id={$user["hashtag_id"]}";
            $hashtag_result = mysqli_query($link, $hashtag_sql);
            
            if(mysqli_num_rows($hashtag_result) > 0){
              if ($row = mysqli_fetch_assoc($hashtag_result)) {
                $hashtag_datas = $row;
                for ($i = 0; $i < count($hashlist);$i++) {
                  if($hashtag_datas["{$hashlist[$i]}"]) {
                    echo "<span class='text-primary'>#{$hashlist[$i]} <span>";
                  }
                }
              }
            }
